...
class Doom3Tokenizer implements IDoom3Tokenizer {
  ...
  private _isDigits(c: string): boolean {
  ...
  public _isWhitespace(c: string): boolean {
  ...
  private getNextToken(tok: IDoom3Token): boolean {
    ...
    let c: string = ""
    // 为什么要reset
    // 因为每次get next token的时候都要清楚上一次token的状态，包括类型，值，然后重新判断
    token.reset()
    do {
      // 拿到不是空白字符串的字符
  ...
  private _getNumber(token: Doom3Token): void {
    ...
    do {
      ...
      c = this._peekChar()
      consumed = true
    } while (c.length > 0 && (this._isDigits(c) || (!isFloat && c === '.')))
